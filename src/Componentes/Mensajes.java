...
//        //Lanzar el Joptionpane
    ...
    public static int ingresar_Monto_a_abonar( int monto)//JoptionPane que solo acepte numeros
    {
        int Monto=monto;
        {
            try {
                Monto = Integer.parseInt(JOptionPane.showInputDialog(null,"INGRESE EL MONTO A ABONAR","CAMBIO DE VALOR", 1));
                if (Monto > monto) {
                    DecimalFormat df = new DecimalFormat("#,###");
                    Sistema("El monto ingresado: "+df.format(Monto)+" supera el valor completo o parcial de esta factura, la cual es de: "+df.format(monto));
                    Monto = Integer.parseInt(JOptionPane.showInputDialog(null,"INGRESE EL MONTO A ABONAR","CAMBIO DE VALOR", 1));
                }else if (Monto == 0) {
                    DecimalFormat df = new DecimalFormat("#,###");
                    Sistema("El monto ingresado: "+df.format(Monto)+" no es aceptado.");
                    Monto = Integer.parseInt(JOptionPane.showInputDialog(null,"INGRESE EL MONTO A ABONAR","CAMBIO DE VALOR", 1));
                }
            } catch (HeadlessException | NumberFormatException e) {
                //informacion("Solo se permiten números");
